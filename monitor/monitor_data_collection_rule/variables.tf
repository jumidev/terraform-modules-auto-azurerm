...
#   streams (list)             : (REQUIRED) Specifies a list of streams. Possible values include but not limited to 'Microsoft-Event', 'Microsoft-InsightsMetrics', 'Microsoft-Perf', 'Microsoft-Syslog', 'Microsoft-WindowsEvent', and 'Microsoft-PrometheusMetrics'.
...
#   streams (list)         : (REQUIRED) Specifies a list of streams that this data source will be sent to. A stream indicates what schema will be used for this data and usually what table in Log Analytics the data will be sent to. Possible value is 'Microsoft-W3CIISLog'.
#   log_directories (list) : Specifies a list of absolute paths where the log files are located.
#
...
#   name (string)            : (REQUIRED) The name which should be used for this data source. This name should be unique across all data sources regardless of type within the Data Collection Rule.
#   streams (list)           : (REQUIRED) Specifies a list of streams that this data source will be sent to. A stream indicates what schema will be used for this data and usually what table in Log Analytics the data will be sent to. Possible values include but not limited to 'Microsoft-Event', 'Microsoft-InsightsMetrics', 'Microsoft-Perf', 'Microsoft-Syslog', 'Microsoft-WindowsEvent'.
#   extension_json (string)  : A JSON String which specifies the extension setting.
#   input_data_sources (list): Specifies a list of data sources this extension needs data from. An item should be a name of a supported data source which produces only one stream. Supported data sources type: 'performance_counter', 'windows_event_log',and 'syslog'.
#
# syslog block structure:
#   facility_names (list) : (REQUIRED) Specifies a list of facility names. Use a wildcard '*' to collect logs for all facility names. Possible values are 'auth', 'authpriv', 'cron', 'daemon', 'kern', 'lpr', 'mail', 'mark', 'news', 'syslog', 'user', 'uucp', 'local0', 'local1', 'local2', 'local3', 'local4', 'local5', 'local6', 'local7',and '*'.
#   log_levels (list)     : (REQUIRED) Specifies a list of log levels. Use a wildcard '*' to collect logs for all log levels. Possible values are 'Debug', 'Info', 'Notice', 'Warning', 'Error', 'Critical', 'Alert', 'Emergency',and '*'.
#   name (string)         : (REQUIRED) The name which should be used for this data source. This name should be unique across all data sources regardless of type within the Data Collection Rule.
#   streams (list)        : Specifies a list of streams that this data source will be sent to. A stream indicates what schema will be used for this data and usually what table in Log Analytics the data will be sent to. Possible values include but not limited to 'Microsoft-Syslog',and 'Microsoft-CiscoAsa', and 'Microsoft-CommonSecurityLog'.
#
# windows_firewall_log block structure:
#   name (string)                       : (REQUIRED) The name which should be used for this data source. This name should be unique across all data sources regardless of type within the Data Collection Rule.
#   streams (list)                      : (REQUIRED) Specifies a list of streams that this data source will be sent to. A stream indicates what schema will be used for this data and usually what table in Log Analytics the data will be sent to.
#
# platform_telemetry block structure:
#   name (string)                     : (REQUIRED) The name which should be used for this data source. This name should be unique across all data sources regardless of type within the Data Collection Rule.
#   streams (list)                    : (REQUIRED) Specifies a list of streams that this data source will be sent to. A stream indicates what schema will be used for this data and usually what table in Log Analytics the data will be sent to. Possible values include but not limited to 'Microsoft.Cache/redis:Metrics-Group-All'.
#
# prometheus_forwarder block structure:
#   name (string)                       : (REQUIRED) The name which should be used for this data source. This name should be unique across all data sources regardless of type within the Data Collection Rule.
#   streams (list)                      : (REQUIRED) Specifies a list of streams that this data source will be sent to. A stream indicates what schema will be used for this data and usually what table in Log Analytics the data will be sent to. Possible value is 'Microsoft-PrometheusMetrics'.
#   label_include_filter (string)       : One or more 'label_include_filter' blocks.
#
...
#   x_path_queries (list)            : (REQUIRED) Specifies a list of Windows Event Log queries in XPath expression. Please see [this document](https://learn.microsoft.com/en-us/azure/azure-monitor/agents/data-collection-rule-azure-monitor-agent?tabs=cli#filter-events-using-xpath-queries) for more information.
#
...
#   counter_specifiers (list)             : (REQUIRED) Specifies a list of specifier names of the performance counters you want to collect. To get a list of performance counters on Windows, run the command 'typeperf'. Please see [this document](https://learn.microsoft.com/en-us/azure/azure-monitor/agents/data-sources-performance-counters#configure-performance-counters) for more information.
#   name (string)                         : (REQUIRED) The name which should be used for this data source. This name should be unique across all data sources regardless of type within the Data Collection Rule.
#   sampling_frequency_in_seconds (number): (REQUIRED) The number of seconds between consecutive counter measurements (samples). The value should be integer between '1' and '300' inclusive. 'sampling_frequency_in_seconds' must be equal to '60' seconds for counters collected with 'Microsoft-InsightsMetrics' stream.
#   streams (list)                        : (REQUIRED) Specifies a list of streams that this data source will be sent to. A stream indicates what schema will be used for this data and usually what table in Log Analytics the data will be sent to. Possible values include but not limited to 'Microsoft-InsightsMetrics',and 'Microsoft-Perf'.
#
