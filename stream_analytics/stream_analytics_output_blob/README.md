...
| **var.batch_max_wait_time** | string |  -  |  -  |  The maximum wait time per batch in `hh:mm:ss` e.g. `00:02:00` for two minutes. | 
| **var.batch_min_rows** | int |  -  |  -  |  The minimum number of rows per batch (must be between `0` and `1000000`). | 
