...
#   days_of_month (number)  : Day(s) of the month on which the trigger is scheduled. This value can be specified with a monthly frequency only.
#   days_of_week (number)   : Days of the week on which the trigger is scheduled. This value can be specified only with a weekly frequency.
#   hours (string)          : Hours of the day on which the trigger is scheduled.
#   minutes (number)        : Minutes of the hour on which the trigger is scheduled.
