...
variable "password" {
  description = "The password to log in the server."
  type        = string
  default     = null
}
variable "user_id" {
  description = "The user ID to log in the server."
  type        = string
  default     = null
}
...
variable "key_vault_password" {
  description = "A 'key_vault_password' block. Use this argument to store Azure File Storage password in an existing Key Vault. It needs an existing Key Vault Data Factory Linked Service."
  type        = map(any)
  default     = null
}
#
