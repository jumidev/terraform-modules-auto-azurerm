...
variable "connection_string" {
  description = "(REQUIRED) The connection string in which to authenticate with Snowflake."
  type        = string
...
variable "key_vault_password" {
  description = "A 'key_vault_password' block. Use this argument to store Snowflake password in an existing Key Vault. It needs an existing Key Vault Data Factory Linked Service."
  type        = map(any)
  default     = null
}
#
