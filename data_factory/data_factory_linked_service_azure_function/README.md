...
| **url** | string |  The url of the Azure Function. | 
...
| **key** | string |  The system key of the Azure Function. Exactly one of either `key` or `key_vault_key` is required | 
| **key_vault_key** | [block](#key_vault_key-block-structure) |  A `key_vault_key` block. Use this Argument to store the system key of the Azure Function in an existing Key Vault. It needs an existing Key Vault Data Factory Linked Service. Exactly one of either `key` or `key_vault_key` is required. | 
