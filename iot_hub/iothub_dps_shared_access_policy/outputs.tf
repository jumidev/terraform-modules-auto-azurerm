output "name" {
  value = azurerm_iothub_dps_shared_access_policy.this.name
}

output "resource_group_name" {
  value = azurerm_iothub_dps_shared_access_policy.this.resource_group_name
}

output "iothub_dps_name" {
  value = azurerm_iothub_dps_shared_access_policy.this.iothub_dps_name
}

output "enrollment_read" {
  value = azurerm_iothub_dps_shared_access_policy.this.enrollment_read
}

output "enrollment_write" {
  value = azurerm_iothub_dps_shared_access_policy.this.enrollment_write
}

output "registration_read" {
  value = azurerm_iothub_dps_shared_access_policy.this.registration_read
}

output "registration_write" {
  value = azurerm_iothub_dps_shared_access_policy.this.registration_write
}

output "service_config" {
  value = azurerm_iothub_dps_shared_access_policy.this.service_config
}

output "id" {
  description = "The ID of the IoTHub Device Provisioning Service Shared Access Policy."
  value       = azurerm_iothub_dps_shared_access_policy.this.id
}

output "primary_key" {
  description = "The primary key used to create the authentication token."
  value       = azurerm_iothub_dps_shared_access_policy.this.primary_key
}

output "primary_connection_string" {
  description = "The primary connection string of the Shared Access Policy."
  value       = azurerm_iothub_dps_shared_access_policy.this.primary_connection_string
}

output "secondary_key" {
  description = "The secondary key used to create the authentication token."
  value       = azurerm_iothub_dps_shared_access_policy.this.secondary_key
}

output "secondary_connection_string" {
  description = "The secondary connection string of the Shared Access Policy."
  value       = azurerm_iothub_dps_shared_access_policy.this.secondary_connection_string
}

