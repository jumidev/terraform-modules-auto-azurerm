...
| **create_mode** | string |  -  |  `Default`, `Restore`  |  The creation mode for the CosmosDB Account. Possible values are `Default` and `Restore`. Changing this forces a new resource to be created. ~> **NOTE:** `create_mode` only works when `backup.type` is `Continuous`. | 
...
| ---- | ---- | --------- | --------- |
| **collection_names** | list | No  | A list of the collection names for the restore request. Changing this forces a new resource to be created. In addition to the Arguments listed above - the following Attributes are exported: | 
