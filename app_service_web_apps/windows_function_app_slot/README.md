...
inputs = {
   name = "Specifies the name of the Windows Function App Slot..."   
...
| ---- | --------- |  ----------- |
| **name** | string |  Specifies the name of the Windows Function App Slot. Changing this forces a new resource to be created. | 
| **function_app_id** | string |  The name of the Windows Function App this Slot is a member of. Changing this forces a new resource to be created. | 
...
| **enabled** | bool |  `True`  |  -  |  Is the Windows Function App Slot enabled. Defaults to `true`. | 
...
| **service_plan_id** | string |  -  |  -  |  The ID of the Service Plan in which to run this slot. If not specified the same Service Plan as the Windows Function App will be used. | 
...
| **storage_key_vault_secret_id** | string |  -  |  -  |  The Key Vault Secret ID, optionally including version, that contains the Connection String to connect to the storage account for this Function App Slot. | 
| **tags** | map |  -  |  -  |  A mapping of tags which should be assigned to the Windows Function App Slot. | 
...
| `oauth_scopes` | list | No | - | Specifies a list of OAuth 2.0 scopes that will be requested as part of Microsoft Account authentication. If not specified, 'wl.basic' is used as the default scope. |
...
| `allowed_audiences` | list | No | - | Specifies a list of Allowed audience values to consider when validating JWTs issued by Azure Active Directory. |
...
| `schedule` | [block](#schedule-block-structure) | Yes | - | a 'schedule' block as detailed below. |
...
| `oauth_scopes` | string | No | - | an 'oauth_scopes'. |

### `site_config` block structure

| Name | Type | Required? | Default | Description |
| ---- | ---- | --------- | ------- | ----------- |
| `always_on` | bool | No | False | If this Windows Web App is Always On enabled. Defaults to 'false'. |
...
| `app_service_logs` | [block](#app_service_logs-block-structure) | No | - | an 'app_service_logs' block as detailed below. |
...
| `application_stack` | [block](#application_stack-block-structure) | No | - | an 'application_stack' block as detailed below. |
| `auto_swap_slot_name` | string | No | - | The name of the slot to automatically swap with when this slot is successfully deployed. |
| `cors` | [block](#cors-block-structure) | No | - | a 'cors' block as detailed below. |
| `default_documents` | list | No | - | Specifies a list of Default Documents for the Windows Web App. |
...
| `ip_restriction` | [block](#ip_restriction-block-structure) | No | - | an 'ip_restriction' block as detailed below. |
...
| `pre_warmed_instance_count` | number | No | - | The number of pre-warmed instances for this function app. Only affects apps on an Elastic Premium plan. |
...
| `scm_ip_restriction` | [block](#scm_ip_restriction-block-structure) | No | - | a 'scm_ip_restriction' block as detailed below. |
...
| `scm_type` | string | No | - | The SCM Type in use by the Windows Function App. |
...
| `windows_fx_version` | string | No | - | The Windows FX Version string. |
...
| `oauth_scopes` | list | No | - | Specifies a list of OAuth 2.0 scopes that will be requested as part of Google Sign-In authentication. If not specified, 'openid', 'profile', and 'email' are used as default scopes. |
...
| `headers` | [block](#headers-block-structure) | No | - | a 'headers' block as detailed below. |
| `ip_address` | string | No | - | The CIDR notation of the IP or IP Range to match. For example: '10.0.0.0/24' or '192.168.10.1/32' |
| `name` | string | No | - | The name which should be used for this 'ip_restriction'. |
...
| `allowed_audiences` | list | No | - | Specifies a list of Allowed Audiences that should be requested as part of Google Sign-In authentication. |
...
| Name | Type | Required? | Default | Description |
| ---- | ---- | --------- | ------- | ----------- |
| `type` | string | Yes | - | Specifies the type of Managed Service Identity that should be configured on this Windows Function App Slot. Possible values are 'SystemAssigned', 'UserAssigned', 'SystemAssigned, UserAssigned' (to enable both). |
| `identity_ids` | list | No | - | A list of User Assigned Managed Identity IDs to be assigned to this Windows Function App Slot. |
...
| `allowed_audiences` | list | No | - | Specifies a list of Allowed Audiences that will be requested as part of Microsoft Sign-In authentication. |
...
| `oauth_scopes` | list | No | - | Specifies a list of OAuth 2.0 scopes to be requested as part of Facebook Login authentication. |

### `application_stack` block structure

| Name | Type | Required? | Default | Description |
| ---- | ---- | --------- | ------- | ----------- |
| `dotnet_version` | string | No | v4.0 | The version of .Net. Possible values are 'v3.0', 'v4.0', 'v6.0', 'v7.0' and 'v8.0'. Defaults to 'v4.0'. |
| `use_dotnet_isolated_runtime` | bool | No | False | Should the DotNet process use an isolated runtime. Defaults to 'false'. |
| `java_version` | string | No | - | The version of Java to use. Possible values are '1.8', '11' and '17' (In-Preview). |
| `node_version` | string | No | - | The version of Node to use. Possible values are '~12', '~14', '~16' and '~18'. |
| `powershell_core_version` | string | No | - | The PowerShell Core version to use. Possible values are '7', and '7.2'. |
| `use_custom_runtime` | bool | No | - | Does the Function App use a custom Application Stack? |
...
| `active_directory` | [block](#active_directory-block-structure) | No | - | an 'active_directory' block as detailed below. |
...
| `facebook` | [block](#facebook-block-structure) | No | - | a 'facebook' block as detailed below. |
| `github` | [block](#github-block-structure) | No | - | a 'github' block as detailed below. |
| `google` | [block](#google-block-structure) | No | - | a 'google' block as detailed below. |
| `issuer` | string | No | - | The OpenID Connect Issuer URI that represents the entity which issues access tokens. |
| `microsoft` | [block](#microsoft-block-structure) | No | - | a 'microsoft' block as detailed below. |
...
| `twitter` | [block](#twitter-block-structure) | No | - | a 'twitter' block as detailed below. |
...
| Name | Type | Required? | Default | Description |
| ---- | ---- | --------- | ------- | ----------- |
| `allowed_origins` | list | No | - | Specifies a list of origins that should be allowed to make cross-origin calls. |
...
| Name | Type | Required? | Default | Description |
| ---- | ---- | --------- | ------- | ----------- |
| `x_azure_fdid` | list | No | - | Specifies a list of Azure Front Door IDs. |
| `x_fd_health_probe` | string | No | - | Specifies if a Front Door Health Probe should be expected. The only possible value is '1'. |
| `x_forwarded_for` | list | No | - | Specifies a list of addresses for which matching should be applied. Omitting this value means allow any. |
| `x_forwarded_host` | list | No | - | Specifies a list of Hosts for which matching should be applied. |
...
| ---- | ---- | --------- | --------- |
| **id** | string | No  | The ID of the Windows Function App Slot | 
...
| **default_hostname** | string | No  | The default hostname of the Windows Function App Slot. | 
| **hosting_environment_id** | string | No  | The ID of the App Service Environment used by Function App Slot. | 
| **identity** | block | No  | An `identity` block. | 
| **kind** | string | No  | The Kind value for this Windows Function App Slot. | 
