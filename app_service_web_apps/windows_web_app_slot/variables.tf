...
variable "name" {
  description = "(REQUIRED) The name which should be used for this Windows Web App Slot. Changing this forces a new Windows Web App Slot to be created."
  type        = string

}
variable "app_service_id" {
  description = "(REQUIRED) The ID of the Windows Web App this Deployment Slot will be part of. Changing this forces a new Windows Web App to be created."
  type        = string
...
#   always_on (bool)                                      : If this Windows Web App Slot is Always On enabled. Defaults to 'true'.
#   api_management_api_id (string)                        : The API Management API ID this Windows Web App Slot os associated with.
#   api_definition_url (string)                           : The URL to the API Definition for this Windows Web App Slot.
...
#   auto_swap_slot_name (string)                          : The Windows Web App Slot Name to automatically swap to when deployment to that slot is successfully completed.
...
#   default_documents (list)                              : Specifies a list of Default Documents for the Windows Web App Slot.
...
#   ip_restriction (string)                               : One or more 'ip_restriction' blocks.
...
#   scm_ip_restriction (string)                           : One or more 'scm_ip_restriction' blocks.
...
#   scm_use_main_ip_restriction (string)                  : Should the Windows Web App Slot 'ip_restriction' configuration be used for the SCM also.
#   use_32_bit_worker (bool)                              : Should the Windows Web App Slotuse a 32-bit worker. Defaults to 'true'.
...
#   worker_count (number)                                 : The number of Workers for this Windows App Service Slot.
#
# action block structure                 :
#   action_type (string)                   : (REQUIRED) Predefined action to be taken to an Auto Heal trigger. Possible values are 'CustomAction', 'LogEvent' and 'Recycle'.
#   custom_action (block)                  : A 'custom_action' block.
#   minimum_process_execution_time (string): The minimum amount of time in 'hh:mm:ss' the Windows Web App Slot must have been running before the defined action will be run in the event of a trigger.
#
...
#   slow_request (string)     : One or more 'slow_request' blocks.
#   status_code (string)      : One or more 'status_code' blocks.
#
...
#   tomcat_version (string)            : The version of Tomcat the Java App should use.
#   java_embedded_server_enabled (bool): Should the Java Embedded Server (Java SE) be used to run the app.
#   java_version (string)              : The version of Java to use when 'current_stack' is set to 'java'. Possible values include '1.7', '1.8', '11' and '17'. Required with 'java_container' and 'java_container_version'.
#   node_version (string)              : The version of node to use when 'current_stack' is set to 'node'. Possible values include '~12', '~14', '~16', and '~18'.
#   php_version (string)               : The version of PHP to use when 'current_stack' is set to 'php'. Possible values are '7.1', '7.4' and 'Off'.
#   python (bool)                      : The app is a Python app. Defaults to 'false'.
#
...
#   enabled (bool)                         : (REQUIRED) Should the Authentication / Authorization feature be enabled for the Windows Web App?
...
#   allowed_external_redirect_urls (string): Specifies a list of External URLs that can be redirected to as part of logging in or logging out of the Windows Web App Slot.
...
#   issuer (string)                        : The OpenID Connect Issuer URI that represents the entity which issues access tokens for this Windows Web App Slot.
#   microsoft (block)                      : A 'microsoft' block.
#   runtime_version (string)               : The RuntimeVersion of the Authentication / Authorization feature in use for the Windows Web App Slot.
...
#   token_store_enabled (bool)             : Should the Windows Web App Slot durably store platform-specific security tokens that are obtained during login flows? Defaults to 'false'.
...
#   oauth_scopes (list)                : Specifies a list of OAuth 2.0 scopes that will be requested as part of Google Sign-In authentication. If not specified, 'openid', 'profile', and 'email' are used as default scopes.
#
...
#   allowed_audiences (list)           : Specifies a list of Allowed audience values to consider when validating JWTs issued by Azure Active Directory.
...
#   oauth_scopes (list)             : Specifies a list of OAuth 2.0 scopes to be requested as part of Facebook login authentication.
#
...
#   oauth_scopes (list)                : Specifies a list of OAuth 2.0 scopes that will be requested as part of Microsoft Account authentication. If not specified, 'wl.basic' is used as the default scope.
#
...
#   oauth_scopes (list)                : Specifies a list of OAuth 2.0 scopes that will be requested as part of GitHub login authentication.
...
#   allowed_audiences (list)           : Specifies a list of Allowed Audiences that should be requested as part of Google Sign-In authentication.
...
#   allowed_audiences (list)           : Specifies a list of Allowed Audiences that will be requested as part of Microsoft Sign-In authentication.
...
variable "enabled" {
  description = "Should the Windows Web App Slot be enabled? Defaults to 'true'."
  type        = bool
  default     = true
}
...
variable "https_only" {
  description = "Should the Windows Web App Slot require HTTPS connections."
  type        = string
  default     = null
}
...
#   type (string)           : (REQUIRED) Specifies the type of Managed Service Identity that should be configured on this Windows Web App Slot. Possible values are 'SystemAssigned', 'UserAssigned', 'SystemAssigned, UserAssigned' (to enable both).
#   identity_ids (list)     : A list of User Assigned Managed Identity IDs to be assigned to this Windows Web App Slot.
...
#   failed_request_tracing (string) : Should failed request tracing be enabled.
...
#   retention_in_days (number)        : (REQUIRED) The time in days after which to remove blobs. A value of '0' means no retention.
...
#   retention_in_days (number) : (REQUIRED) The retention period in days. A values of '0' means no retention.
#   retention_in_mb (number)   : (REQUIRED) The maximum size in megabytes that log files can use.


variable "service_plan_id" {
  description = "The ID of the Service Plan in which to run this slot. If not specified the same Service Plan as the Windows Web App will be used."
  type        = string
  default     = null
}
...
variable "tags" {
  description = "A mapping of tags which should be assigned to the Windows Web App Slot."
  type        = map(any)
  default     = null
}
