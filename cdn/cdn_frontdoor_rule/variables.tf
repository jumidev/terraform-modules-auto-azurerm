...
#   query_string_parameters (list)                     : A list of query string parameter names. ->**NOTE:** 'query_string_parameters' is a required field when the 'query_string_caching_behavior' is set to 'IncludeSpecifiedQueryStrings' or 'IgnoreSpecifiedQueryStrings'.
#   compression_enabled (bool)                         : Should the Front Door dynamically compress the content? Possible values include 'true' or 'false'. ->**NOTE:** Content won't be compressed on AzureFrontDoor when requested content is smaller than '1 byte' or larger than '1 MB'.
