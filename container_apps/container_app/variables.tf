...
#   ip_security_restriction (string) : One or more 'ip_security_restriction' blocks for IP-filtering rules as defined below.
#   target_port (string)             : (REQUIRED) The target port on the container for the Ingress traffic.
#   exposed_port (string)            : The exposed port on the container for the Ingress traffic. ~> **Note:** 'exposed_port' can only be specified when 'transport' is set to 'tcp'.
#   traffic_weight (string)          : (REQUIRED) One or more 'traffic_weight' blocks as detailed below.
...
variable "workload_profile_name" {
  description = "The name of the Workload Profile in the Container App Environment to place this Container App. ~> **Note:** Omit this value to use the default 'Consumption' Workload Profile."
  type        = string
  default     = null
}
variable "tags" {
