...
#   identity_ids (list)     : (REQUIRED) Specifies a list of User Assigned Managed Identity IDs to be assigned to this Batch Account.
...
#   resource_file (string)                : One or more 'resource_file' blocks that describe the files to be downloaded to a compute node as defined below.
#
...
#   container_registries (string)          : One or more 'container_registries' blocks. Additional container registries from which container images can be pulled by the pool's VMs. Changing this forces a new resource to be created.
...
#   endpoint_configuration (string)          : A list of 'endpoint_configuration' blocks that can be used to address specific ports on an individual compute node externally as defined below. Set as documented in the inbound_nat_pools block below. Changing this forces a new resource to be created.
