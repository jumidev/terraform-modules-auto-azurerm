...
| **var.admin_ssh_key** | block | False | -  |  -  |  One or more `admin_ssh_key` blocks. Changing this forces a new resource to be created. | | `admin_ssh_key` block structure: || 
|   public_key (string): (REQUIRED) The Public Key which should be used for authentication, which needs to be at least 2048-bit and in 'ssh-rsa' format. Changing this forces a new resource to be created. ||
|   username (string): (REQUIRED) The Username for which this Public SSH Key should be configured. Changing this forces a new resource to be created. ||
