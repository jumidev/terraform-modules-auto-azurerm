...
#   script_actions (block)        : The script action which will run on the cluster. One or more 'script_actions' blocks.
#   number_of_disks_per_node (int): (REQUIRED) The number of Data Disks which should be assigned to each Worker Node, which can be between 1 and 8. Changing this forces a new resource to be created.
