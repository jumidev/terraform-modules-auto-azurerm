...
resource "azurerm_hdinsight_kafka_cluster" "this" {
  ...
  component_version {
    kafka = lookup(component_version.value, "kafka") # (Required) 
  }
  ...
  roles {
    ...
    kafka_management_node = lookup(roles.value, "kafka_management_node", null)
  }
  ...
  dynamic "rest_proxy" { # var.rest_proxy
    for_each = var.rest_proxy != null ? var.rest_proxy : []
    content {
      security_group_id   = lookup(rest_proxy.value, "security_group_id")   # (Required) 
      security_group_name = lookup(rest_proxy.value, "security_group_name") # (Required) 
    }
  }
