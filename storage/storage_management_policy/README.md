...
| `match_blob_index_tag` | block | No | - | A 'match_blob_index_tag' block. The block defines the blob index tag based filtering for blob objects. |
...
| Name | Type | Required? | Default | Description |
| ---- | ---- | --------- | ------- | ----------- |
| `base_blob` | block | No | - | A 'base_blob' block. |
| `snapshot` | block | No | - | A 'snapshot' block. |
| `version` | block | No | - | A 'version' block. |
