...
#   root_certificate (string)                        : One or more 'root_certificate' blocks which are defined below. These root certificates are used to sign the client certificate used by the VPN clients to connect to the gateway.
#   revoked_certificate (string)                     : One or more 'revoked_certificate' blocks which are defined below.
...
#   sa_lifetime_in_seconds (number)   : (REQUIRED) The IPSec Security Association lifetime in seconds for a Site-to-Site VPN tunnel. Possible values are between '300' and '172799'.
#   sa_data_size_in_kilobytes (number): (REQUIRED) The IPSec Security Association payload size in KB for a Site-to-Site VPN tunnel. Possible values are between '1024' and '2147483647'.
#
...
#   score (number)               : (REQUIRED) The score of the Radius Server determines the priority of the server. Possible values are between '1' and '30'.
