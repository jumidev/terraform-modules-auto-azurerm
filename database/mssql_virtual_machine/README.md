...
| `max_dop` | number | No | 0 | Maximum Degree of Parallelism of the SQL Server. Possible values are between '0' and '32767'. Defaults to '0'. |
| `max_server_memory_mb` | number | No | 2147483647 | Maximum amount memory that SQL Server Memory Manager can allocate to the SQL Server process. Possible values are between '128' and '2147483647' Defaults to '2147483647'. |
| `min_server_memory_mb` | number | No | 0 | Minimum amount memory that SQL Server Memory Manager can allocate to the SQL Server process. Possible values are between '0' and '2147483647' Defaults to '0'. |
...
| `log_backup_frequency_in_minutes` | number | Yes | - | Frequency of log backups, in minutes. Valid values are from '5' to '60'. |
| `days_of_week` | number | No | - | A list of days on which backup can take place. Possible values are 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday' and 'Sunday' |
...
| `manual_schedule` | [block](#manual_schedule-block-structure) | No | - | A 'manual_schedule' block. When this block is present, the schedule type is set to 'Manual'. Without this block, the schedule type is set to 'Automated'. |
| `retention_period_in_days` | number | Yes | - | Retention period of backups, in days. Valid values are from '1' to '30'. |
