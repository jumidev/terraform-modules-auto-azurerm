...
variable "server_id" {
  description = "(REQUIRED) The id of the MS SQL Server on which to create the database. Changing this forces a new resource to be created. ~> **NOTE:** This setting is still required for 'Serverless' SKUs"
  type        = string
...
variable "geo_backup_enabled" {
  description = "A boolean that specifies if the Geo Backup Policy is enabled. Defaults to 'true'. ~> **NOTE:** 'geo_backup_enabled' is only applicable for DataWarehouse SKUs (DW*). This setting is ignored for all other SKUs."
  type        = bool
  default     = true
}
