...
| `user_assigned_identity_id` | string | No | - | The User Assigned Identity ID to be used for accessing the Customer Managed Key for encryption. |
...
| ---- | ---- | --------- | --------- |
| **git_url** | string | No  | Specifies the GitHub Enterprise host name. For example: <https://github.mydomain.com>. -> **Note:** You must log in to the Synapse UI to complete the authentication to the GitHub repository. In addition to the Arguments listed above - the following Attributes are exported: | 
